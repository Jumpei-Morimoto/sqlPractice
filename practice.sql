...
SELECT name ,life_expectancy,indep_year FROM countries WHERE life_expectancy is not null ORDER BY life_expectancy DESC,indep_year ASC;
...
SELECT Min(surface_area) AS 'アジアの最小面積'FROM countries WHERE continent ='Asia';
...
SELECT countries.name,countrylanguages.language  FROM countries join countrylanguages on countrylanguages.country_code=countries.code;

-- 問26
-- 全ての国と言語と市区町村を表示してください。
SELECT countries.name AS '国名',cities.name AS '市区町村',countrylanguages.language AS 'language' FROM countries join countrylanguages on countrylanguages.country_code=countries.code join cities on cities.country_code = countries.code;
...
SELECT celebrities.name,countries.name ,countrylanguages.language FROM celebrities JOIN countries on countries.code = celebrities.country_code join countrylanguages on countrylanguages.country_code=countries.code where countrylanguages.is_official='T';

-- 問29
-- 全ての有名人の名前と国名をに出力してください。 ただしテーブル結合せずサブクエリを使用してください。
SELECT name,(select name FROM countries GROUP BY code HAVING(countries.code=celebrities.country_code)) AS '国名' from celebrities;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT country_code,MAX(age),MIN(age) from celebrities where  not(country_code='') group by country_code having max(age)>=50 and min(age)<=30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT '1980'AS'誕生月',count(birth) AS '人数' FROM celebrities WHERE birth like '%1980%'  UNION SELECT '1981'AS'誕生月',count(birth) AS '人数' FROM celebrities WHERE birth like '%1981%' ;

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT name AS '国名',(select avg(age)from celebrities group by country_code HAVING(countries.code=celebrities.country_code)) AS '平均年齢'  FROM countries where EXISTS(SELECT * FROM celebrities WHERE countries.code=celebrities.country_code) ORDER BY (select avg(age)from celebrities group by country_code HAVING(countries.code=celebrities.country_code)) DESC;
